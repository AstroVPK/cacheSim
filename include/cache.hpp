...
char LRUCache<num_cache_sets, num_cache_ways, address_size_bits, cacheline_length_bytes>::read(std::size_t address) {
	...
	for (std::size_t way_num = 0; way_num < num_ways; ++way_num) {
		if (valid[way_num][index] == true) {
			if (tag_table[way_num][index] == tag) {
				clk_table[way_num][index] = clk;
				data = data_table[way_num][index][offset];
				found = true;
			}
		}
	...
	if (found == false) {
		...
		for (std::size_t off = 0; off < cacheline_length; ++off) {
			data_table[lru_way][index][off] = mem[wrapped_addr_start + off];	
		}

		data = data_table[lru_way][index][offset];
	}
...
char LRUCache<num_cache_sets, num_cache_ways, address_size_bits, cacheline_length_bytes>::read_debug(std::size_t address) {
	...
	for (std::size_t way_num = 0; way_num < num_ways; ++way_num) {
		...
		std::cout << "valid[" << way_num << "][" << index << "]: " << valid[way_num][index] << std::endl;
		if (valid[way_num][index] == true) {
			std::cout << "tag_table[" << way_num << "][" << index << "]: " << tag_table[way_num][index] << " == tag: " << tag << " is " << (tag_table[way_num][index] == tag) << std::endl;
			if (tag_table[way_num][index] == tag) {
				clk_table[way_num][index] = clk;
	...
	if (found == false) {
		...
		std::cout << "valid[" << lru_way << "][" << index << "]: " << valid[lru_way][index] << std::endl;
		std::cout << "clk_table[" << lru_way << "][" << index << "]: " << clk_table[lru_way][index] << std::endl;
		std::cout << "tag_table[" << lru_way << "][" << index << "]: " << tag_table[lru_way][index] << std::endl;
